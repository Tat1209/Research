    ...
    for di in [0.002, 0.004, 0.006, 0.008, 0.01, 0.015, 0.02, 0.03, 0.05, 0.075, 0.1, 0.015, 0.2, 0.3, 0.5, 0.75, 1.0]:
        with mlflow.start_run(run_name=f"{fi}") as run:
            ...
            for e in range(epochs):
                ...
                test_loss, test_acc = model.train_1epoch(train_loader, mixup=mixup)
                val_loss, val_acc = model.val_1epoch(val_loader)

                met_dict = {"epoch":e+1, "test_loss":test_loss, "test_acc":test_acc, "val_loss":val_loss, "val_acc":val_acc}
