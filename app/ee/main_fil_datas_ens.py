...
mlflow.set_experiment("datas_fils_nens")
# mlflow.set_experiment("tmp")

# 中断したとこ
# di=0.002, fils=4, ti=1, batch_size=125, ensembles=256

for bi in range(2):
    for ti in range(2):
        for fi in [2 ** i for i in range(6, -1, -1)]:
            for ei in [4 ** i for i in range(int(6 + 1 - math.log2(fi)))]:
                for di in [0.002, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 0.75, 1.0]:
                    with mlflow.start_run(run_name=f"{fi}") as run:
                        mlflow.log_metric("max_lr",     max_lr := 0.005)   # Adam
                        mlflow.log_metric("epochs",     epochs := 100)
                        if bi == 0: mlflow.log_metric("batch_size", batch_size := 125)
                        elif bi == 1: mlflow.log_metric("batch_size", batch_size := int(di * 50000 / 100))
                        mlflow.log_metric("fils",       fils := fi)
                        mlflow.log_metric("ensembles",  ensembles := min([ei, 2048]))
                        # mlflow.log_metric("ensembles",  ensembles := min([(64 // ei) ** 2, 2048]))
                        ...
                        print(f"{di=}, {fils=}, {ti=}, {batch_size=}, {ensembles=}")
