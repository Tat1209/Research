...
for fi in [2 ** i for i in range(6, -1, -1)]:
    with mlflow.start_run(run_name=f"{fi}") as run:
        ...
        for _ in range(1):
            ...
            model = Model(network, loss_func, optimizer=optimizer, scheduler_t=scheduler_t, device=device)
            models.append(model)
        ens = Ens(models)

        # ens.load_sds('tmp.pkl')

        hp_dict = {"loss_func":repr(ens.models[0].loss_func), "optimizer":repr(ens.models[0].optimizer), "scheduler":utils.sched_repr(ens.models[0].scheduler_t[0])}
        mlflow.log_params(hp_dict)
        mlflow.log_metric("params", ens.count_params())
        mlflow.log_text(repr(ens.models[0].network), "model_layers.txt")
        ...
        for e in range(epochs):
            ...
            tLoss, tAcc = ens.me_train_1epoch(train_loader, mixup=mixup)
        ...
        # ens.save_sds('tmp.pkl')
        # ens.mlflow_save_sds(mlflow)
