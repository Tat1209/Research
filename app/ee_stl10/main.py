...
for base_fi in [32, 16, 8, 4, 2]:
    for fi in [[2 ** i for i in range(int(math.log2(base_fi)) + 1)]]:
        for di in [0.1, 0.08, 0.06, 0.04, 0.02, 0.01]:
            ...
            runs_mgr.log_param("train_trans", repr(train_trans := Trans.stl_git_32))
            runs_mgr.log_param("val_trans", repr(val_trans := Trans.stl_gen_32))

            train_loader = dl(ds("stl10_train", train_trans).balance_label().in_range(di), batch_size, shuffle=True)
            val_loader = dl(ds("stl10_test", val_trans), batch_size, shuffle=True)
