...
for lr in [0.0001]:
    ...
    run_mgr.log_param("epochs", epochs := 5)
    run_mgr.log_param("batch_size", batch_size := 256)
    ...
    labeled_ds = ds("ai-step_l", u_seed=0)
    ...
    train_ds, val_ds = labeled_ds.split(ratio=0.7, shuffle=False, balance_label=True)
    ...
    val_loader = dl(val_ds, batch_size=256, shuffle=True)
    test_loader = dl(test_ds, batch_size=256, shuffle=True)
    ...
    run_mgr.log_param("model_arc", net.__name__)
